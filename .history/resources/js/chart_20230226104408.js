...
// Define a function to retrieve the transaction data from the server
function getTransactionData() {
    return axios.get('/transactions');
}

// Create a function to format the transaction data for use in the chart
function formatTransactionData(data) {
    ...
    data.forEach((row) => {
        ...
        payments.push(row.total_payments);
    });
    return { labels, counts, payments };
}

// Create a function to initialize the transaction chart
function initTransactionChart() {
    // Get the transaction data from the server
    getTransactionData()
        .then((response) => {
            // Format the transaction data
            let data = formatTransactionData(response.data);

            // Create the transaction chart
            let ctx = $('#transaction-chart')[0].getContext('2d');
...
// Call the initTransactionChart function to create the transaction chart on page load
