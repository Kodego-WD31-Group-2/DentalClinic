...
<div class="flex items-center justify-center">
  <label for="status" class="block text-sm font-medium text-custom-2 p-2">Appointment Status:</label>
  <select id="status" name="status" class="form-select mt-2 sm:mr-2" aria-label="Default select example">
...
<div class="form-group">
  <label for="status" class="block text-sm font-medium text-custom-2 p-2">Appointment Status:</label>
  <select id="status" name="status" class="form-select mt-1 block w-full border-blue-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500" required>
    <option>{{ old('status', $appointment->status) }}</option>
    <option value="Morning" {{ old('status') == 'Morning' ? 'selected' : '' }}>Morning</option>
    <option value="Afternoon" {{ old('status') == 'Afternoon' ? 'selected' : '' }}>Afternoon</option>
  </select>
</div>  
