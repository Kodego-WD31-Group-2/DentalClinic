                    ...
                    <thead>
                        <tr>
                            ...
                            @foreach (range(-2, 2) as $i)
                                <th>{{ now()->addMonths($i)->format('M Y') }}</th>
                            @endforeach
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($transactions->groupBy('payment_method') as $paymentMethod => $data)
                            <tr>
                                <td>{{ $paymentMethod }}</td>
                                @foreach (range(-2, 2) as $i)
                                    @php
                                        $monthData = $data->where('month', now()->addMonths($i)->month)
                                                           ->where('year', now()->addMonths($i)->year)
                                                           ->first();
                                    @endphp
                                    <td>{{ $monthData ? $monthData->count : 0 }}</td>
                                @endforeach
                            </tr>
                        @endforeach
                    </tbody>
