...
$transactionsByMonth = [];
foreach ($paidTransactions->concat($pendingTransactions)->concat($failedTransactions) as $transaction) {
    $month = $transaction->transaction_date->format('Y-m');
    $amount = 0;
    foreach ($transaction->transactionItems as $item) {
        $amount += $item->feeSchedule->amount;
    }
    ...
    $transactionsByMonth[$month]['amount'] += $amount;
}
