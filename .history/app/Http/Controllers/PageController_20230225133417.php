...
class PageController extends Controller
{
    ...
    {
        //Today's Appointment
        // $transactionsToday = Appointment::whereDate('appointment_date', today())->count();
        // $transactionsPendingToday = Appointment::whereDate('appointment_date', today())->where('status', 'pending')->count();
        // $transactionsCompletedToday = Appointment::whereDate('appointment_date', today())->where('status', 'completed')->count();
        // $transactionsCancelledToday = Appointment::whereDate('appointment_date', today())->where('status', 'cancelled')->count();
        ...
        foreach ($transactions as $transaction) {
            foreach ($transaction->transactionItems as $item) {
                $total += $item->feeSchedule->amount;
            }
        }

        
        $today = Carbon::today();
        $transactionsPaidToday = Transaction::where('payment_status', 'paid')
            ->whereDate('created_at', $today)
            ->sum('total_cost');
    ...
    //     $totalCostToday = Transaction::whereDate('created_at', today())
    // ->where('status', 'paid')
    // ->sum('total_cost');
        ...
        return view('pages/dashboard-overview-3', [
            ...
            // 'transactionsCompletedToday' => $transactionsCompletedToday,
            // 'transactionsCancelledToday' => $transactionsCancelledToday,
            // Specify the base layout.
            // Eg: 'side-menu', 'simple-menu', 'top-menu', 'login'
            // The default value is 'side-menu'
            'appointmentsThisMonth' => $appointmentsThisMonth,
