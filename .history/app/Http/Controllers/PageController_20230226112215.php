...
->whereBetween('transaction_date', [
    now()->subMonths(2)->startOfMonth()->format('Y-m-d'),
    now()->addMonths(2)->endOfMonth()->format('Y-m-d'),
])
->groupBy('month', 'year', 'payment_method')
->orderBy('year', 'asc')
->orderBy('month', 'asc')
->orderBy('payment_method', 'asc')
->get();

    // Get the last 6 months and add them to the $months collection
    for ($i = 0; $i < 6; $i++) {
    ...
    // Get the count of transactions for each payment method and month
    $query = DB::table('transactions')
        ->select(DB::raw('payment_method'), DB::raw('DATE_FORMAT(transaction_date, "%Y-%m") as month'), DB::raw('COUNT(*) as count'))
        ->whereIn('payment_method', $paymentMethods)
        ->whereBetween('transaction_date', [now()->subMonths(5)->startOfMonth(), now()->endOfMonth()])
        ->groupBy('payment_method', 'month')
        ->get();

    // Convert the query results to a collection
    foreach ($query as $row) {
        $transactions->put($row->month, [
            $row->payment_method => $row->count,
        ] + ($transactions->get($row->month) ?? []));
    }
