...
{       
    ...
    $transactionsPaidToday = Transaction::where('payment_status', 'paid')
        ...
        ->reduce(function ($total, $transaction) {
            return $total + $transaction->transactionItems->sum(function ($item) {
                return $item->feeSchedule->amount;
            });
        }, 0);

    // Pending transactions today
    $transactionsPendingToday = Transaction::where('payment_status', 'pending')
        ->whereDate('created_at', today())
        ->count();

    // Failed transactions today
    $transactionsFailedToday = Transaction::where('payment_status', 'failed')
        ->whereDate('created_at', today())
        ->count();

    // Overall total payments
    $totalPayments = Transaction::where('payment_status', 'paid')
        ->with('transactionItems.feeSchedule')
        ->get()
        ->reduce(function ($total, $transaction) {
            return $total + $transaction->transactionItems->sum(function ($item) {
