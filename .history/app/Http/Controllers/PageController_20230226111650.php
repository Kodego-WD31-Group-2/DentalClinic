...
// Get the previous 2 months
$previousMonth1 = date('Y-m-01', strtotime('-1 month'));
$previousMonth2 = date('Y-m-01', strtotime('-2 month'));

// Get the next 2 months
$nextMonth1 = date('Y-m-01', strtotime('+1 month'));
$nextMonth2 = date('Y-m-01', strtotime('+2 month'));

// Query the database for the transactions by payment method and month
$transactions = DB::table('transactions')
    ->select(DB::raw('payment_method, DATE_FORMAT(transaction_date, "%Y-%m") as month'), DB::raw('COUNT(*) as count'))
    ->whereBetween('transaction_date', [$previousMonth2, $nextMonth2])
    ->groupBy('payment_method', 'month')
    ->get();

// Group the transactions by payment method and month
$transactionsByMethodAndMonth = [];
foreach ($transactions as $transaction) {
    if (!isset($transactionsByMethodAndMonth[$transaction->payment_method])) {
        $transactionsByMethodAndMonth[$transaction->payment_method] = [];
    }
    $transactionsByMethodAndMonth[$transaction->payment_method][$transaction->month] = $transaction->count;
}
        ...
        return view('pages/dashboard-overview-3', [
            ...
            'transactionsByMethodAndMonth' => $transactionsByMethodAndMonth,
